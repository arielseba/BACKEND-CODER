...
// DELETE PARA ELIMINAR UN PRODUCTO POR ID  ---------REVISAR MENSAJE DE RESPUESTA DE VALIDATION ID
router.delete("/:id", async (req, res) => {
  let productos = await archivo.getAll();
  const id = Number(req.params.id);
  const validationId = productos.find((note) => note.id === id);
  if (validationId) {
    archivo.deleteById(id);
    return res.status(200).json({ deleted: validationId });
  }
  return res
    .status(400)
    .json({ error: "No se encontro el producto que desea eliminar" });
});
// PUT
router.put("/:id", upload.single("imgProduct"), async (req, res) => {
  const file = req.file;
  const id = Number(req.params.id);
  ...
  if (producto) {
    const productoActualizado = {
      ...
      img: file || "No hay imagen del producto",
    };
    const respuesta = await archivo.updateById(id, productoActualizado);
    return res.json({ update: respuesta });
  }
});
// POST
